...
void *upo_ht_sepchain_put(upo_ht_sepchain_t ht, void *key, void *value)
{
    ...
    if (ht != NULL && ht->slots != NULL)
    {
        ...
        for (node = ht->slots[hash].head; node != NULL && ht->key_cmp(key, node->key) != 0; node = node->next)
        {
        }
        if (node == NULL)
        {
...
void *upo_ht_sepchain_get(const upo_ht_sepchain_t ht, const void *key)
{
    ...
    for (node = ht->slots[h].head; node != NULL && ht->key_cmp(node->key, key) != 0; node = node->next)
    {
    }
...
void upo_ht_sepchain_delete(upo_ht_sepchain_t ht, const void *key, int destroy_data)
{
    size_t h = ht->key_hash(key, ht->capacity);
    upo_ht_sepchain_list_node_t *node = NULL;
    upo_ht_sepchain_list_node_t *temp = NULL;
...
void *upo_ht_linprob_put(upo_ht_linprob_t ht, void *key, void *value)
{
    ...
    if (ht != NULL && ht->slots != NULL)
    {
        ...
        while ((ht->slots[h].key != NULL && ht->key_cmp(ht->slots[h].key, key) != 0) || ht->slots[h].tombstone != 0)
        {
            if (ht->slots[h].tombstone != 0 && !foundTomb)
            {
                foundTomb = 1;
                tombSlot = h;
            }
...
{
    if (upo_ht_linprob_get(ht, key) != NULL)
    {
        return 1;
    }
